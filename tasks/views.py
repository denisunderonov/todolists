...
from rest_framework.permissions import IsAuthenticated  # Проверка аутентификации
from django_filters.rest_framework import DjangoFilterBackend  # Фильтрация
from rest_framework.filters import SearchFilter, OrderingFilter  # Поиск и сортировка
...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user.is_authenticated:
            queryset = queryset.filter(
                Q(project__owner=self.request.user) |  # ИЛИ владелец проекта
                Q(assigned_to=self.request.user) |  # ИЛИ задача назначена пользователю
                Q(created_by=self.request.user)  # ИЛИ пользователь создал задачу
            )
    ...
    def overdue(self, request):
        ...
        tasks = self.get_queryset().filter(
            Q(due_date__lt=timezone.now()) &  # Срок выполнения в прошлом
            ~Q(status__name='Завершена')  # НЕ статус "Завершена" (используем NOT ~)
        )
    ...
    def urgent_or_tomorrow(self, request):
        ...
        tasks = self.get_queryset().filter(
            (Q(priority__level__gte=4) & ~Q(status__name='Завершена')) |  # (Приоритет >= 4 И НЕ завершена) ИЛИ
            Q(due_date__range=[tomorrow_start, tomorrow_end])  # Срок выполнения завтра
        )
    ...
    def others_in_progress_or_cancelled(self, request):
        ...
        tasks = Task.objects.filter(
            ~Q(created_by=request.user) &  # НЕ созданные текущим пользователем
            (Q(status__name='В работе') | Q(status__name='Отменена'))  # И (статус "В работе" ИЛИ "Отменена")
        )
